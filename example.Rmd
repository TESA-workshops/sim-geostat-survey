...
First we will simulate a spatial, age-structured population with SimSurvey. Default settings for the `sim_abundance()` and `sim_distribution()` function are largely used
...
ggsave("presentation/graphics/sim-sp-survey.svg", width = 9, height = 6)
```
...
ggsave("presentation/graphics/sim-sp-abund.svg", width = 9, height = 6)
```
...
filter(both, year %in% 1:5) %>% 
  ...
  facet_grid(type ~ year) +
  scale_fill_viridis_c(limits = quantile(log(both$N_scaled), probs = c(0.05, 1)))
ggsave("presentation/graphics/sim-sp-fit.svg", width = 10, height = 4)
```
...
ggplot(both, aes(year, N_scaled, group = type)) +
  ...
  theme_minimal()
ggsave("presentation/graphics/sim-ts-1.svg", width = 6, height = 4)
```
