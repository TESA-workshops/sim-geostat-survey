...
Next we will conduct a survey on our simulated population. We will work with a single replicate.

```{r}
survey <- SimSurvey::sim_survey(sim, n_sims = 1)
```

And, again, they format the resulting data into a data frame we can work with:
...
- `include_spatial = TRUE` means the model will include a separate spatial random field (a wiggly intercept surface that stays constant through time) and spatiotemporal fields (a ranfom field that varies each year). There's also an option for correlated fields (`ar1_fields = TRUE`).
...
These are randomized quantile residuals as are used in the DHARMa R package. These are adjusted so that they would be Gaussian distributed if the model were perfectly consistent with the data:
