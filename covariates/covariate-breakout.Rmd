...
plot_grid(survey_trim) + scale_fill_viridis_c(direction = -1)
```
...
```{r}
  survey <- sim_survey(sim, n_sims = 1, min_sets = 20
    ) %>% run_strat()
  xy <- as_tibble(survey$grid_xy)
  dat <- as_tibble(survey$setdet) %>%
    dplyr::select(x, y, set, year, N = n, tow_area)
...
nd$offset <- mean(dat$offset)
 ...
 y <- sim_parabola(mu = 200, sigma = 25, plot=F)(x = 50:500)
  tru_curve <- data.frame(x = 50:500, y)  
