...
sim_and_fit <- function(iter) {
  ...
  ## Example of visualizing the conditional effect of depth:
...
result_scaled <- result %>%
  ...
  mutate(
    ...
    type = factor(type, levels =
        c("True", "Design-based", "Model-based", "Model-based-depth"))
  ) %>%
  ungroup() %>%
  arrange(type)
...
ggplot(summary_stats1, aes(log(true), log(est))) +
  geom_point() +
  coord_fixed() +
  geom_abline(intercept = 0, slope = 1) +
  theme_bw()

ggplot(summary_stats2, aes(log(true), log(est))) +
  geom_point() +
  coord_fixed() +
  geom_abline(intercept = 0, slope = 1) +
  theme_bw()
