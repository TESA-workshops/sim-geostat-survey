...
For this breakout group, we proposed joining two major ongoing software development efforts at DFO---one a simulation tool (SimSurvey)^7^ and one a model fitting tool (sdmTMB)^1,8^ --- to use simulation testing to address several questions around the use of geostatistical model-based calibration of survey data.
...
1.  Does including a covariate (e.g., depth) increase precision, estimate the right depth relationship, and maintain an unbiased index with greater precision compared to not including any covariates?
    Are there scenarios where that is or isn't the case?
...
The subgroup working on joining or stitching nearby surveys (subgroup coding led by Q. Huynh) designed a series of experiments to evaluate the effects of sampling from two possible areas in various years.
Here, we focus on two scenarios: (Q2) surveys A and B have different catchabilities and each sample different areas in alternate years and (Q4) same as Q2 but the terminal year includes samples from both surveys A and B (Fig. \@ref(fig:cal-year)).
...
\@ref(fig:stitchQ4)).
Subsequent work has suggested that other structural forms to the random fields can improve stitching performance further.
...
Challenges included the limited amount of time for the project (\~ 6 hours), the virtual setting, challenges for users working with packages that they were not yet familiar with, computational demands give in the limited time, visualizing high dimensional problems and summarizing results.
...
Despite the challenges and complexity of the focal questions, preliminary results highlight the potential utility of these tools for testing the efficacy of model-based solutions to common survey problems.
...
Reduced or partial coverage introduces uncertainty and preliminary results from this workshop indicate that sdmTMB can either reduce the uncertainty by accounting for the effects of a covariate or by propagating uncertainty over missing areas, it can produce a better representation of uncertainty.

# Next steps

With limited time, we only scratched the surface of the focal questions.
More rigorous testing of each question is required to draw more conclusive conclusions.
...
Both sdmTMB and SimSurvey, as well as the repository associated with this breakout session, are made as open and accessible as possible to allow the broader community to validate, reuse and improve.
We hope that the open-source nature of code behind these simulation analyses facilitate further collaborative research to tackle some of these questions.

# References

1.  Anderson, S. C., Keppel, E. A. & Edwards, A. M. A reproducible data synopsis for over 100 species of British Columbia groundfish. *DFO Can. Sci. Advis. Sec. Res. Doc.* **2019/041**, vii + 321 p (2019).
2.  Shelton, A. O., Thorson, J. T., Ward, E. J. & Feist, B. E. Spatial semiparametric models improve estimates of species abundance and distribution. *Can. J. Fish. Aquat. Sci.* **71**, 1655--1666 (2014).
3.  Maureaud, A. A. *et al.* Are we ready to track climate-driven shifts in marine species across international boundaries? - A global survey of scientific bottom trawl data. *Glob. Change Biol.* **27**, 220--236 (2021).
4.  Grüss, A. & Thorson, J. T. Developing spatio-temporal models using multiple data types for evaluating population trends and habitat usage. *ICES J. Mar. Sci.* **76**, 1748--1761 (2019).
5.  Thorson, J. T., Shelton, A. O., Ward, E. J. & Skaug, H. J. Geostatistical delta-generalized linear mixed models improve precision for estimated abundance indices for West Coast groundfishes. *ICES J. Mar. Sci. J. Cons.* **72**, 1297--1310 (2015).
6.  Haggarty, D. R. *et al.* Evaluation of potential rebuilding strategies for Inside Yelloweye Rockﬁsh (*Sebastes ruberrimus*) in British Columbia. *DFO Can. Sci. Advis. Sec. Res. Doc.* **In press**, (2020).
7.  Regular, P. M. *et al.* SimSurvey: An R package for comparing the design and analysis of surveys by simulating spatially-correlated populations. *PLOS ONE* **15**, e0232822 (2020).
8.  Anderson, S. C., Ward, E. J., Barnett, L. A. K. & English, P. A. *sdmTMB: Spatiotemporal species distribution GLMMs with 'TMB'*. (2021). <https://github.com/pbs-assess/sdmTMB>
