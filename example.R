...
sim <- SimSurvey::sim_abundance(ages = seq(1, 10), years = seq(1, 10)) %>%
  SimSurvey::sim_distribution(grid = SimSurvey::make_grid(res = c(10, 10), depth_range = c(10, 500)),
...
ggplot(df, aes(x, y, fill = log(N + 1))) + geom_tile() +
  facet_grid(age~year) +
  scale_fill_viridis_c()

# SA: weird edge effects?
df_sum <- dplyr::filter(df_sum, x > -110, x < 110)
ggplot(df_sum, aes(x, y, fill = log(N + 1))) + geom_tile() +
  facet_wrap(~year) +
  scale_fill_viridis_c()

ggplot(df_sum, aes(x, y, fill = depth)) + geom_tile() +
  facet_wrap(~year) +
...
ggplot(dat, aes(x, y, fill = log(N1000))) + geom_tile() +
  facet_wrap(~year) +
  scale_fill_viridis_c()
...
fit <- sdmTMB(N1000 ~ 0 + as.factor(year), data = dat,
