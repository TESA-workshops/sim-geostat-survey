...
## Breakout questions {.build}

- Can biased inference be avoided using contemporary models?
- Can models fill the gaps left by flaws in survey design?
...
## What's [SimSurvey](https://github.com/PaulRegular/SimSurvey)? {.smaller}
...
## What's [sdmTMB](https://github.com/pbs-assess/sdmTMB)? {.smaller}
...
1. **Make a mesh of discrete locations** - `make_mesh`
    - Used to estimate continuous spatial process

2. **Fit spatiotemporal GLMM** - `sdmTMB`
    - Implements a geostistical model using TMB, R-INLA, and Gaussian Markov random fields
    
3. **Diagnose model** - `residuals`
    - Check residuals using built in methods
...
- **Next steps:** 
  ...
  2. Introduce survey problems (e.g. reduce spatial coverage) and and repeat 1.
  
- Initial testing can be found [here](https://github.com/TESA-workshops/sim-geostat-survey), where:
